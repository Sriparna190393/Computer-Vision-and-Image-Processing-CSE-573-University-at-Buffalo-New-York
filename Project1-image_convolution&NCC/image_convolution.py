...
def convolve2d(img, kernel):
    ...
    new_padded_image = utils.zero_pad(img, 1, 1)
    img_conv = copy.deepcopy(img)
    xmax = 3
    for xmin in range(0, len(new_padded_image), 1):
        if xmax < len(img):
            ymin = 0
            ymax = 3
            while ymax < len(new_padded_image):
                cropped_image = utils.crop(new_padded_image, xmin, xmax, ymin, ymax)
                multi_res = utils.elementwise_mul(cropped_image, flip_kernel)
                add_res = sum(sum(multi_res, []))
                img_conv[xmin][ymin] = round(add_res)       
                ymin = ymin + 1
                ymax = ymax + 1
        xmax = xmax + 1
    return img_conv
